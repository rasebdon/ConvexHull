...
std::vector<Vector2> quickhull::Execute(const std::vector<Vector2> &points) const
{
    ...
    quickHull(points, points[min_x], points[max_x], convexHull);

    return convexHull;
}

double quickhull::findDistance(Vector2 p1, Vector2 p2, Vector2 p) {
...
void quickhull::quickHull(const std::vector<Vector2> &points, Vector2 p1, Vector2 p2, std::vector<Vector2> &result) {
    int index = -1;
